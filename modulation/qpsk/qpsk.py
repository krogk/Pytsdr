...
def ModulationQPSK(data, bitPeriod, fc):
    ...
    for i in range(0,nBits,2):
        ...
        for j in range(bitPeriod):
            # Compute Amplitude
            # Summation: I + Q 
            txModulated = np.append(txModulated, ((2*data[i]-1)*np.cos(2*np.pi*fc*t)) + ((2*data[qIndex]-1)*np.sin(2*np.pi*fc*t)))
...
def DemodulationQPSK(txModulated, fc, nBits, bitPeriod, b1Filt, nTaps):
    ...
    iDemodulated = np.append(iDemodulated, -np.ones(nTaps//2))
    qDemodulated = np.append(qDemodulated, -np.ones(nTaps//2))
